...
class EXTRACT_META(object):
    ...
    def transform_neck_weight(self):
        ...
        mc.skinCluster('head_lod0_mesh_skinCluster', e=True, lw=True, ai='neck_01')  # 将neck_01加入到模型的蒙皮里
        for jnt in mc.skinCluster('head_lod0_mesh_skinCluster', inf=1, q=1):  # 全锁所有关节
            mc.setAttr('{}.liw'.format(jnt), True)

        mc.setAttr('neck_01.liw', False)  # 单开脖子关节
        skin_jnt_lis = mc.skinCluster('head_lod0_mesh_skinCluster', inf='', q=True)#获取所有该蒙皮节点影响的关节
