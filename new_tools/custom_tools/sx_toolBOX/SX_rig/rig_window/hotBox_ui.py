...
class SX_RotBox(object):
    ...
    def _buildMenu(self, menu, *args):
        ...
        mc.menuItem(p=sundry_menu, l=u'清理选中对象的orig节点', rp='NE', c='from sx_toolBOX.SX_rig import rig_tool;'
                                                                           'reload(rig_tool);'
                                                                           'rig_tool.clear_orig()')
        mc.menuItem(p=sundry_menu, l=u'冻结选中关节的旋转', rp='W', c='from sx_toolBOX.SX_rig import rig_tool;'
                                                                      'reload(rig_tool);'
        ...
        mc.menuItem(p=except_menu, l=u'导出当前选择为fbx', rp='S', c='from sx_toolBOX.SX_rig import rig_tool;'
                                                                    'reload(rig_tool);'
                                                                    'rig_tool.exportSelectToFbx();')
        mc.menuItem(p=except_menu, l=u'清理场景重名', rp='N', c='from sx_toolBOX.SX_rig import rig_clear;'
                                                                'reload(rig_clear);'
                                                                'rig_clear.clear_name()')
        mc.menuItem(p=except_menu, l=u'检查可能的uv位置错误', rp='NW', c='from sx_toolBOX.SX_rig import rig_clear;'
                                                                        'reload(rig_clear);'
                                                                        'rig_clear.clear_uv(0)')
        mc.menuItem(p=except_menu, l=u'清理场景关键帧', rp='NE', c='from sx_toolBOX.SX_rig import rig_clear;'
                                                                  'reload(rig_clear);'
                                                                  'rig_clear.clear_key()')
        mc.menuItem(p=except_menu, l=u'清理场景动画层', rp='E', c='from sx_toolBOX.SX_rig import rig_clear;'
                                                                  'reload(rig_clear);'
                                                                  'rig_clear.clear_animLayer()')
        mc.menuItem(p=except_menu, l=u'清理场景HIK', rp='SE', c='from sx_toolBOX.SX_rig import rig_clear;'
                                                                'reload(rig_clear);'
                                                                'rig_clear.clear_hik()')
        ...
        mc.menuItem(p=get_menu, l=u'选中个数与对象', rp='NW', c='from sx_toolBOX.SX_rig import rig_tool;'
                                                                'reload(rig_tool);'
                                                                'rig_tool.get_length()')
        mc.menuItem(p=get_menu, l=u'选中同名节点', rp='SW', c='from sx_toolBOX.SX_rig import rig_tool;'
                                                              'reload(rig_tool);' 
