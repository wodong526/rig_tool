...
class TreeWidget(ItemViewMixin, QtWidgets.QTreeWidget):
    ...
    def createHeaderMenu(self, column):
        ...
        action = menu.addAction(u"隐藏 '{0}'".format(label))
        callback = partial(self.setColumnHidden, column, True)
        action.triggered.connect(callback)

        menu.addSeparator()

        action = menu.addAction(u'根据内容调整大小')
    ...
    def createHideColumnMenu(self):
        ...
        :rtype: QtWidgets.QMenu
        """
        menu = QtWidgets.QMenu(u"显示/隐藏列", self)

        action = menu.addAction(u"显示所有")
        action.triggered.connect(self.showAllColumns)

        action = menu.addAction(u"隐藏所有")
    ...
    def createCopyTextMenu(self):
        ...
        :rtype: QtWidgets.QMenu
        """
        menu = QtWidgets.QMenu(u"复制文本", self)
        ...
        if self.selectedItems():
            ...
            action = menu.addAction(u"没有项被选择")
