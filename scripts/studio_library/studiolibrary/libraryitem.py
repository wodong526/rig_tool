...
class LibraryItem(studiolibrary.widgets.Item):
    ...
    def createOverwriteMenu(self, menu):
        ...
        if not self.isReadOnly():
            menu.addSeparator()
            action = QtWidgets.QAction(u"覆盖", menu)
    ...
    def showRenameDialog(self, parent=None):
        ...
        if button == QtWidgets.QDialogButtonBox.Ok:
            try:
                ...
                self.showExceptionDialog(u"重命名错误。", error)
                raise
    ...
    def showMoveDialog(self, parent=None):
        ...
        if dst:
            ...
            try:
                ...
                self.showExceptionDialog(u"移动错误", error)
                raise
    ...
    def showDeleteDialog(self):
        ...
        button = self.showQuestionDialog(u"删除项目", text)

        if button == QtWidgets.QDialogButtonBox.Yes:
            try:
                self.delete()
            except Exception as error:
                self.showExceptionDialog(u"删除错误", error)
                raise
